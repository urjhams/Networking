...
public extension BaseRequest {
  ...
  private func generateHawkHeader(
    ...
    let resource = urlComponents.path + (urlComponents.query?.isEmpty == false ? "?\(urlComponents.query!)" : "")
    ...
    // Generate MAC
    let mac = try generateHawkMAC(normalizedString: normalizedString, key: key, algorithm: algorithm)
  ...
  private func generateAWSSignatureHeaders(
    ...
    let authorization = "AWS4-HMAC-SHA256 Credential=\(accessKey)/\(credentialScope), SignedHeaders=\(getSignedHeaders(awsHeaders)), Signature=\(signature)"
  ...
  private func generateOAuth1SignatureBaseString(
    ...
    let baseUrl = "\(urlComponents.scheme ?? "https")://\(urlComponents.host ?? "")\(urlComponents.path)"
  ...
  private func generateOAuth1Signature(
    ...
    case .hmacSha1:
      ...
      let signature = HMAC<Insecure.SHA1>.authenticationCode(for: baseData, using: SymmetricKey(data: keyData))
      return Data(signature).base64EncodedString()
      
    case .hmacSha256:
      let keyData = signingKey.data(using: .utf8)!
      let baseData = baseString.data(using: .utf8)!
      let signature = HMAC<SHA256>.authenticationCode(for: baseData, using: SymmetricKey(data: keyData))
      return Data(signature).base64EncodedString()
  ...
  private func generateHawkMAC(
    ...
    switch algorithm {
    case .sha1:
      let mac = HMAC<Insecure.SHA1>.authenticationCode(for: stringData, using: SymmetricKey(data: keyData))
  ...
  private func createAWSCanonicalRequest(
    ...
    let canonicalQueryString = urlComponents.queryItems?.sorted { $0.name < $1.name }
